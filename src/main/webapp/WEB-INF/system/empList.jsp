    ...
    <base href="<%=basePath%>" />
		...
		<script type="text/javascript">
			...
			function deleteEmp(empId){
				alert("123");
				var flag =window.confirm("确认要删除嘛");
				if(flag){
						...
						<a>
							<select class="select1" name="deptno">
								...
								<c:forEach items="${findAll}" var="f">
									<c:if test="${f.deptno == deptno }">
										<option value="${f.deptno}" selected="selected">${f.deptName}</option>
									</c:if>
									<c:if test="${f.deptno != deptno }">
										<option value="${f.deptno}">${f.deptName}</option>
									</c:if>
